...
- **整理转换**：把收集箱里的灵感原地转为四类正式事务——`Next Actions` / `Project` / `Waiting for` / `Someday`，不新建重复记录。
- **四类视图 + Project 树**：
...
- **快捷键**：`1`–`5` 切换视图，`Enter` 快速新增 / 保存，`Esc` 关闭弹窗。
...
| Inbox 收集箱 | 今日启动弹窗 |
|---|---|
| ![Inbox 收集箱](docs/screenshots/screenshot-inbox.png) | ![今日启动弹窗](docs/screenshots/screenshot-today.png) |
...
- Windows Installer 包：`src-tauri/target/release/bundle/msi/NowTree_1.0.0_x64_en-US.msi`
- NSIS 安装包：`src-tauri/target/release/bundle/nsis/NowTree_1.0.0_x64-setup.exe`
...
| 按键 | 作用 |
|---|---|
| `1` / `2` / `3` / `4` / `5` | 切换 Inbox / Next Actions / Projects / Waiting for / Someday |
...
- **Habits 每日打卡**：重复性例行项与 GTD 一次性事务区分，互补于 Stats 的连续数据
